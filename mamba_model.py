    ...
    def forward(
        ...
        if gate is not None:
            ...
            if w_sl.shape[0] == batch_size:
                gate = F.gumbel_softmax(torch.log(torch.cat([w_sl, 1.0 - w_sl], dim=2) + 1e-10) * 10, tau=1, dim=2,
                                        hard=False)[:, :, 0]
...
class Mamba_sflow(nn.Module):
    ...
    def forward(
        ...
        if gate is not None:
            ...
            if w_sl.shape[0] == batch_size:
                gate = F.gumbel_softmax(torch.log(torch.cat([w_sl, 1.0 - w_sl], dim=2) + 1e-10), tau=1, dim=2,
                                        hard=False)[:, :, 0]
                # gate = F.gumbel_softmax(torch.cat([w_sl,1.0-w_sl],dim=2)*10, tau = 1, dim = 2, hard = False)[:,:,0]
                gate_mask = gate
            else:
